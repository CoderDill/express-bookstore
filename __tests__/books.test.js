...
beforeEach(async () => {
  let result = await db.query(`
    INSERT INTO
      ...
      VALUES(
        '4545454545',
        'https://amazon.com/coderdill',
        'CoderDill',
        'English',
        1,
        'Dill publishing',
        'My autobiography', 2021)
      RETURNING isbn`);
...
describe("POST /books", function () {
  test("Creates a new book", async function () {
    const response = await request(app).post(`/books`).send({
      isbn: "6565656565",
      amazon_url: "https://coderdill.com",
      author: "CoderDill",
      language: "english",
      pages: 100,
      publisher: "Dill publishing",
  ...
  test("Prevents creating book without required title", async function () {
    const response = await request(app).post(`/books`).send({ year: 2012 });
